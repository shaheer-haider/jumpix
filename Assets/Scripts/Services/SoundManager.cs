...
namespace SgLib
{
    ...
    public class SoundManager : MonoBehaviour
    {
        ...
        public List<Sound> backgroundMusic;
        ...
        bool isBackgorundMusicPlayed = false;
        ...
        // play backgorund music coroutine
        IEnumerator PlayBackgroundMusic()
        {
            while (true)
            {
                if (musicState == PlayingState.Playing && !IsMusicOff())
                {
                    PauseMusic();
                }
                else if (GameManager.Instance.GameState != GameState.Playing &&
                    !IsMusicOff() &&
                    !isBackgorundMusicPlayed)
                {
                    if (GameManager.Instance.GameState == GameState.GameOver && (GameManager.Instance.lastGameOverTime + 7f) > Time.time)
                    {
                        yield return null;
                    }
                    else if (musicState == PlayingState.Stopped)
                    {
                        Sound bgMusic = backgroundMusic[Random.Range(0, backgroundMusic.Count)];
                        audioSource.clip = bgMusic.clip;
                        audioSource.Play();
                        isBackgorundMusicPlayed = true;
                    }
                }
                else if (GameManager.Instance.GameState == GameState.Playing)
                {
                    isBackgorundMusicPlayed = false;
